...
| `0.6.0-dev` | 6.0.2 | `wrynose` | A006 is in progress. The approved line adds a deterministic, read-only diagnostics CLI and closed schema 1 without changing the source lock, lab manifests, guest interfaces, or runtime-evidence schemas. No publication, merge, tag, or release is yet claimed. |
