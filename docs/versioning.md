...
| `0.5.0-dev` | 6.0.2 | `wrynose` | A005 clean build revision `782eb6d` completed all 4,702 ARM64 image tasks; clean runtime subject `2e4f87d` then passed all nine ARM64 project cases and all 19 unchanged PCI project cases under software QEMU with validated platform-v1 and PCI-v3 evidence. Those subjects predate replay of the unpublished A005 history onto public A004 closeout `01ff717`, so they remain provisional local evidence rather than final qualification. Exact-revision Linux and metadata evidence plus required reviews remain open. |
